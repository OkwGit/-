...
def create_long_image(folder_path):
    # Define the tag to identify processed images
    processed_tag = "processed_"
    
    # 获取所有未处理的PNG和JPG文件
    valid_extensions = ['.png', '.jpg', '.jpeg']
    files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if os.path.splitext(f)[1].lower() in valid_extensions and not f.startswith(processed_tag)]
    num_files = len(files)
    if num_files == 0:
    ...
    # images = [im.resize((max_width, int(im.height * max_width / im.width)), Image.ANTIALIAS) for im in images]
    ...
    # Move and tag the original images
    archive_folder_path = os.path.join(folder_path, "OG Archived")
    os.makedirs(archive_folder_path, exist_ok=True)
    for file in files:
        base_name = os.path.basename(file)
        new_name = processed_tag + base_name
        shutil.move(file, os.path.join(archive_folder_path, new_name))

    print(f"\033[96m长图保存完成，文件路径：{output_path}\033[0m")
    print("\033[92m所有原始图片已经被移动并标记到'OG Archived'文件夹.\033[0m")
